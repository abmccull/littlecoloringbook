...
#
# Overrides APP_URL to https://littlecolorbook.com for the production env
# and http://localhost:3000 for development. Everything else copies verbatim
...
while IFS= read -r line || [[ -n "$line" ]]; do
  ...
  case "$key" in
    APP_URL)
      case "$ENV_TARGET" in
        production) value="https://littlecolorbook.com" ;;
